"""Cascade Mask R-CNN on UBC roof_coarse task (5 coarse roof classes)."""
_base_ = './cascade-mask-rcnn_r50_fpn_ubc_base.py'

dataset_type = 'UBCRoofCoarseDataset'
train_ann = 'annotations/roof_coarse_train.json'
val_ann = 'annotations/roof_coarse_val.json'
num_classes = 5
...
# Cascade has 3 bbox heads with different regression target_stds — all 3
# and the mask head need num_classes updated.
model = dict(
    roi_head=dict(
        bbox_head=[
            dict(
